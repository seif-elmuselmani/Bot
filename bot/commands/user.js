...
const registerUserCommands = (bot) => {
  ...
  bot.action('services_info', async (ctx) => {
    try {
      ...
      const infoMessage =
        ...
        `  3. عند موافقتك ودفع التكلفة المطلوبة، يبدأ العمل فوراً وتسليم الملفين (ملف الورد المعدل + ملف القياس) بعد انتهائه.`;
  ...
  const showInstructionsMenu = async (ctx) => {
    const instructionMessage = 
      ...
      `• تقليل نسبة الذكاء الاصطناعي ✍️ — <b>تسعير يدوي</b>\n\n` +
      `💡 <b>طريقة الاستخدام:</b>\n` +
      ...
      `• <b>ملاحظة عن عدد الكلمات:</b> أنظمة فحص التشابه (Turnitin) لا تقبل أي ملف يقل عن <b>300 كلمة</b> وسيتم رفضه تلقائياً.\n\n` +
      `❓ <b>أهم الأسئلة الشائعة:</b>\n` +
      `🔸 <b>أشحن بكام عشان أقدر أستخدمه؟</b>\n` +
      `  - اشحن بالرصيد اللي يناسبك، وأسعار الخدمات تبدأ من 45 نقطة فقط.\n` +
